        ...
        return {
            ...
            link: function ($scope, element, attr) {
                        ...
                        .on("changed.jstree", function (e, data) {
                            ...
                            if (!wasInTreeChangeEvent) {
                                ...
                                if (feature && (!feature.inputType || feature.inputType.name == 'CHECKBOX')) {
                                    var value = data.node.state.selected ? 'true' : 'false';
                            ...
                            "types": {
                                "default": {
                                    "icon": "icon-folder tree-item-icon-color icon-lg"
                                },
                                "file": {
                                    "icon": "icon-file tree-item-icon-color icon-lg"
                                }
                            },
                            'checkbox': {
                    ...
                    $tree.find('.jstree-node').each(function () {
                        ...
                        if (feature.inputType.name == 'CHECKBOX') {
                            ...
                            if (!$nodeLi.find('.feature-tree-textbox').length) {
                                ...
                                var $textbox = $('<input class="feature-tree-textbox" type="' + inputType + '" />').val(featureValue);
                            ...
                            if (!$nodeLi.find('.feature-tree-combobox').length) {
                                ...
                                $combobox.val(featureValue).on('change', function () {
                ...
                function findFeatureByName(featureName) {
                    var feature = _.find($scope.editData.features, function (f) {
                        return f.name == featureName;
                    });

                    if (!feature) {
                        infrastructure.log.warn('Could not find a feature by name: ' + featureName);
                    }
                    return feature;
                }
                ...
                function setFeatureValueByName(featureName, value) {
                    var featureValue = _.find($scope.editData.featureValues, function (f) {
                        return f.name == featureName;
                    });
