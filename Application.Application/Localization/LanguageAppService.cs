...
using Application.Localization.Dto;
using Application.Localization.Dto;
...
{
    public class LanguageAppService : ApplicationAppServiceBase, ILanguageAppService
    {
        ...
        {
            var languages = (await _applicationLanguageManager.GetLanguagesAsync(InfrastructureSession.TenantId)).OrderBy(l => l.DisplayName);
            var defaultLanguage = await _applicationLanguageManager.GetDefaultLanguageOrNullAsync(InfrastructureSession.TenantId);

            return new GetLanguagesOutput(languages.MapTo<List<ApplicationLanguageListDto>>(),defaultLanguage == null ? null : defaultLanguage.Name);
        }

        [InfrastructureAuthorize(AppPermissions.PagesAdministrationHostLanguages)]
        public async Task<GetLanguageForEditOutput> GetLanguageForEdit(NullableIdDto input)
        {
            ...
            //Flags
            output.Flags = CultureFlagsHelper
                .FlagClassNames
                .OrderBy(f => f)
                .Select(f => new ComboboxItemDto(f, CultureFlagsHelper.GetCountryCode(f)) { IsSelected = output.Language.Icon == f })
                .ToList();
            return output;
        }
        ...
        public async Task DeleteLanguage(EntityDto input)
        {
            ...
            await _applicationLanguageManager.RemoveAsync(InfrastructureSession.TenantId, language.Name);
        }
        ...
        public async Task SetDefaultLanguage(SetDefaultLanguageInput input)
        {
            await _applicationLanguageManager.SetDefaultLanguageAsync(
                InfrastructureSession.TenantId,
        ...
        public async Task<PagedResultDto<LanguageTextListDto>> GetLanguageTexts(GetLanguageTextsInput input)
        {
            ...
            if (input.BaseLanguageName.IsNullOrEmpty())
            {
                ...
                {
                    defaultLanguage = (await _applicationLanguageManager.GetLanguagesAsync(InfrastructureSession.TenantId)).FirstOrDefault();
            ...
            var languageTexts = source
                ...
                .Select(localizedString => new LanguageTextListDto
                {
                    ...
                    BaseValue = _applicationLanguageTextManager.GetStringOrNull(InfrastructureSession.TenantId, source.Name, baseCulture, localizedString.Name),
                    TargetValue = _applicationLanguageTextManager.GetStringOrNull(InfrastructureSession.TenantId, source.Name, targetCulture, localizedString.Name, false)
                })
                .AsQueryable();
            ...
            {
                languageTexts = languageTexts.Sorting(input.Sorting);
            }
        ...
        public async Task UpdateLanguageText(UpdateLanguageTextInput input)
        {
            ...
            var source = LocalizationManager.GetSource(input.SourceName);
            await _applicationLanguageTextManager.UpdateStringAsync(InfrastructureSession.TenantId, source.Name, culture, input.Key, input.Value);
        }
        ...
        protected virtual async Task UpdateLanguageAsync(CreateOrUpdateLanguageInput input)
        {
            ...
            await _applicationLanguageManager.UpdateAsync(InfrastructureSession.TenantId, language);
        }
        ...
        {
            var existingLanguage = (await _applicationLanguageManager.GetLanguagesAsync(InfrastructureSession.TenantId))
