...
{
    public class ExcelHelper: ApplicationAppServiceBase
    {
        ...
        public FileDto SaveToExcel<T>(IEnumerable<T> data, string fileName, string OpenPassword = "")
        {
            var fileDto = new FileDto(fileName, MimeTypeNames.ApplicationVndOpenxmlformatsOfficedocumentSpreadsheetmlSheet);
            var filePath = Path.Combine(AppFolders.TempFileDownloadFolder, fileDto.FileToken);
            FileInfo fileInfo = new FileInfo(filePath);

            using (ExcelPackage excelPackage = new ExcelPackage(fileInfo, OpenPassword))
            {
                ExcelWorksheet excelWorksheet = excelPackage.Workbook.Worksheets.Add(typeof(T).Name);
                excelWorksheet.OutLineApplyStyle = true;

                excelWorksheet.Cells["A1"].LoadFromCollection(data, true, TableStyles.Medium10);
        ...
        public IEnumerable<T> LoadFromExcel<T>(string FileName) where T : new()
        {
