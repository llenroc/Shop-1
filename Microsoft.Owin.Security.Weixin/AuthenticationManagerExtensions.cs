...
namespace Microsoft.Owin.Security
{
    public static class AuthenticationManagerExtensions
    {
        ...
        public static async Task<Dictionary<string, string>> GetExternalWeixinLoginInfoAsync(this IAuthenticationManager manager, string externalAuthenticationType)
        {
            ...
            if (result != null && result.Identity != null && result.Identity.FindFirst(WeixinSecurityConstants.WeixinClaimType) != null)
            {
                var value = result.Identity.FindFirst(WeixinSecurityConstants.WeixinClaimType).Value;
